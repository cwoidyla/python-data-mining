...
from sklearn.cluster import Birch
start = time.time()
brc = Birch(n_clusters=6, compute_labels=True)
brc.fit(trimData)
end = time.time()
print(end - start)
# %%
predictions = brc.predict(trimData)
# %%view data
...
# %%
cllabels = pd.DataFrame({'a':brc.labels_})
cllabels.head(10)
cllabels.tail(10)
cllabels['a'].value_counts()
# %%
indexRange = np.where(labels=="1")
cllabels = pd.DataFrame({'a':brc.labels_[indexRange]})
cllabels['a'].value_counts()
...
normalized_mutual_info_score(labels, brc.labels_) # overall
indexRange = np.where(labels=="1")
normalized_mutual_info_score(labels[indexRange], brc.labels_[indexRange]) # FARM
indexRange = np.where(labels=="2")
normalized_mutual_info_score(labels[indexRange], brc.labels_[indexRange]) # GRHM
indexRange = np.where(labels=="3")
normalized_mutual_info_score(labels[indexRange], brc.labels_[indexRange]) # HTFD
indexRange = np.where(labels=="4")
normalized_mutual_info_score(labels[indexRange], brc.labels_[indexRange]) # MRRY
indexRange = np.where(labels=="5")
normalized_mutual_info_score(labels[indexRange], brc.labels_[indexRange]) # PGHL
indexRange = np.where(labels=="6")
normalized_mutual_info_score(labels[indexRange], brc.labels_[indexRange]) # RSVL
